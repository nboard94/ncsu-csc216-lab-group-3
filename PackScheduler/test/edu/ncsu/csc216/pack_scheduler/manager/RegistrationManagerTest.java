...
public class RegistrationManagerTest {
	...
	/**
	 * test for getCourseCatalog()
	 */
	@Test
	public void testGetCourseCatalog() {
		manager.getCourseCatalog().loadCoursesFromFile("test-files/course_records.txt");
		String[][] s = manager.getCourseCatalog().getCourseCatalog();
		assertEquals("CSC116", s[0][0]);
		assertEquals("001", s[0][1]);
		assertEquals("Intro to Programming - Java", s[0][2]);
		assertEquals("MW 9:10AM-11:00AM", s[0][3]);
	}

	/**
	 * test for getStudentDirectory()
	 */
	@Test
	public void testGetStudentDirectory() {
		manager.getStudentDirectory().loadStudentsFromFile("test-files/student_records.txt");
		String[][] s = manager.getStudentDirectory().getStudentDirectory();
		assertEquals("Demetrius", s[0][0]);
		assertEquals("Austin", s[0][1]);
		assertEquals("daustin", s[0][2]);
	}
	...
	public void testLogin() {
		// test exist student with wrong password
		manager.getStudentDirectory().loadStudentsFromFile("test-files/student_records.txt");
		assertFalse(manager.login("daustin", "password"));

		// add new student and test login in
		...
		// test login as Registrar with wrong password
		manager.logout();
		assertFalse(manager.login("registrar", "password"));

		// test cannot login when other user in logged in
		assertTrue(manager.login("registrar", "Regi5tr@r"));
		assertFalse(manager.login("jaustin", "password"));

		// test null id, or password
		manager.logout();
	...
	public void testEnrollStudentInCourse() {
		...
		directory.loadStudentsFromFile("ts-test-files/student_records.txt");
